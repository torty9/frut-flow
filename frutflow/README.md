...
| First dictation is slow after opening the lid | The current build automatically coalesces closed-lid maintenance wakes, refreshes audio once, and warms the model after a visible wake — but never *ahead of* a clip you already dictated (the clip itself pages the model in). Open `~/.flowdictate/flow.log` if this still repeats. |
| It still says **Listening** after you let go of the key | macOS occasionally disables a hotkey tap for a moment (a stalled process after wake, secure input) and the key‑up is delivered without us. A modifier hotkey is now observed through a passive tap that macOS never disables for slowness, and while you hold the key the app also checks the *physical* key state, so a lost key‑up ends the capture within about half a second instead of at the 120 s cap. If it ever recurs, `flow.log` (now timestamped) says `tap disabled by macOS (…)` or `release never reached the event tap` next to it. |
| **Transcribing…** lingers after the text already landed | Post‑paste bookkeeping (undo safety, learning from your edits) reads the focused field through Accessibility; those reads are now capped at 1 s each and the edit diff runs off the dictation thread. A genuinely slow clip logs `model=…s` in its `timing` line — several seconds right after a wake is the model paging back in, not a hang. |
